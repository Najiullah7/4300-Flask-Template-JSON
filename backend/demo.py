...
def svd_top_k(df, query, vectorizer, words, docs_normed, data, index_to_word, k = 10):
    ...
    for i in range(k):
        ...
        curr_name = rf['name'][i].lower()
        if curr_name in fav_pokemon:
            pop = fav_pokemon[curr_name]
        popularity.append(pop)
    rf['pop'] = pd.Series(popularity)
